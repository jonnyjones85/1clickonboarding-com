#!/usr/bin/env python3
"""
revert_cta.py — Revert all 20 CTA blocks to match original GHL site exactly.

Changes:
1. Remove .cta-email-form wrapper div, set iframe to original attributes (height:188px, no negative margin)
2. Restore "256-bit security encryption" text + authorized_payments badge
3. Make guarantee text ALL CAPS with original styling
4. Restore checkout badge original inline styles
5. Remove .cta-email-form CSS blocks
"""
...
# The CURRENT CTA inner content (from <p class="toptxt"> through closing tags)
# This appears in all 20 blocks with minor variations in the H1 text
# We'll match the standardized portion that's identical across all blocks
...
def main():
    ...
    # Match from <div class="top_right_sec big_cta"> through its content to closing </div>s
    # The pattern: opening div, then toptxt paragraph, then inner_white_bkg with all content,
    # closing </div> for inner_white_bkg and </div> for top_right_sec
    cta_pattern = re.compile(
        r'(<div class="top_right_sec big_cta">)\s*'
        r'<p class="toptxt">DIGITAL DOWNLOAD NOW AVAILABLE</p>'
        r'.*?'  # All inner content (non-greedy)
        r'</div>\s*</div>',  # Close inner_white_bkg + close top_right_sec
        re.DOTALL
    )
    ...
    print(f"After replacement: {count_after} CTA blocks (all now identical)")

    # Verify no .cta-email-form wrapper divs remain
    wrapper_count = html.count('cta-email-form')
    print(f"Remaining .cta-email-form references: {wrapper_count}")

    # Step 2: Remove .cta-email-form CSS blocks (2 occurrences)
    ...
    # Verify CSS removed
    css_remaining = html.count('.cta-email-form')
    print(f"Remaining .cta-email-form CSS references: {css_remaining}")

    # Verify key elements are present
    print(f"\nVerification:")
    print(f"  '256-bit security encryption' count: {html.count('256-bit security encryption')}")
    print(f"  'authorized_payments' count: {html.count('authorized_payments')}")
    print(f"  'BACKED BY OUR UNCONDITIONAL' count: {html.count('BACKED BY OUR UNCONDITIONAL')}")
    print(f"  'height:188px' count: {html.count('height:188px')}")
    print(f"  'data-height=\"432\"' count: {html.count('data-height=\"432\"')}")
    print(f"  'cta-email-form' count: {html.count('cta-email-form')}")
